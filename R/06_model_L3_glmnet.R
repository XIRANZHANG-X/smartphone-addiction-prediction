...
fit_predict <- function(X_tr, y_tr, X_va) {
  ...
  # glmnet 完全不能容忍 NA，一个都不行 —— 所以这条路径只对
  # 做过插补的线（L2/L3/L4）有意义。L1 会在这里直接报错，那是预期行为。
  ...
  # 3 折内部交叉验证挑 lambda。glmnet 默认会标准化，不需要手动做。
  cvfit <- glmnet::cv.glmnet(
    Xtr, y_tr,
    family = "binomial",
    alpha  = 0.5,        # elastic net：一半 L1 一半 L2
    nfolds = 3,
    type.measure = "auc"
  )

  # 注意：predict.cv.glmnet 是 S3 方法，不是导出对象，不能用 glmnet:: 直接取。
  # cv.glmnet 调用已经加载了 glmnet 命名空间，S3 分发会自动找到它。
  as.numeric(predict(cvfit, newx = Xva, s = "lambda.min", type = "response"))
}
