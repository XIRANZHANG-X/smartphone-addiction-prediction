...
if (sys.nframe() == 0L || identical(environment(), globalenv())) {
  if (!file.exists("output/oof")) {
    ...
    # ⚠ 别把倍数写死在这里。选哪三对是按当前 AUC 排名动态定的，
    #   换一批全量结果，跨度就变 —— 2026-09-01 那一版是 9 倍，
    #   加入逐取值编码重跑之后是 5.8 倍（0.000098 对 0.000564）。
    #   所以只陈述结构性事实，倍数由上面逐对打印的数字自己说话。
    cat("\n=> 同一份数据上，最接近的一对与跨模型族的一对，下限相差数倍。\n")
