...
# ⚠ 口径声明（2026-09-01）
# -----------------------------------------------------------------------------
# 本脚本自己写 CV 循环，**不走 06_framework.R**，因此它不做逐取值 target encoding。
# 换句话说，它产出的是「加入编码之前」的口径。
#
# output/ablation.rds 里现存的数字（例如「删掉 notifications 掉 0.00825」）
# 描述的是一个当前代码已不再产生的 31 特征配置。它们仍然是**当初做特征取舍的
# 依据**，所以保留；但不要拿它们描述现在的模型。
#
# 加入编码后，原始列的重要性被编码列大量吸收（见 R/13_importance.R 的新表：
# 每日屏幕时间的置换重要性从 0.1332 掉到 0.0061），所以在新配置下重跑这份消融，
# 数字很可能明显不同。尚未重跑，约需 3 小时。
