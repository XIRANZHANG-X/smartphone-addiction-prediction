# =============================================================================
# 18_constraint_faithfulness.R —— 插补方法对硬约束有多不老实
#
# 用法：Rscript R/18_constraint_faithfulness.R
# 产出：output/constraint_faithfulness.rds、控制台报表
#
# -----------------------------------------------------------------------------
# 要回答的问题
# -----------------------------------------------------------------------------
# 发现 1：daily_screen_time_hours >= social_media_hours + gaming_hours
#         在 451,246 行完整数据上 100% 成立，是结构性硬约束，不是统计相关。
#
...
# ---- 基准：完整数据上约束确实 100% 成立（复核发现 1）------------------------
cc <- dt0[!is.na(daily_screen_time_hours) & !is.na(social_media_hours) &
          !is.na(gaming_hours)]
viol_cc <- cc[, mean(daily_screen_time_hours < social_media_hours + gaming_hours - 1e-6)]
hr("基准：完整数据上的约束违反率（复核发现 1）")
cat(sprintf("完整行数：%s，违反率：%.4f%%（应接近 0）\n",
            format(nrow(cc), big.mark = ","), 100 * viol_cc))

# ---- 标记「本来就没法算」的行：三列里至少一列缺失 ---------------------------
na_screen <- is.na(dt0$daily_screen_time_hours)
na_social <- is.na(dt0$social_media_hours)
na_gaming <- is.na(dt0$gaming_hours)
needs_impute <- na_screen | na_social | na_gaming

hr("参与审查的行")
cat(sprintf("三列（屏幕/社交/游戏）至少缺一个、需要插补才能凑出约束的行：%s（%.1f%%）\n",
            format(sum(needs_impute), big.mark = ","), 100 * mean(needs_impute)))

check_violation <- function(dt, na_screen, na_social, na_gaming, needs_impute) {
  v <- dt$daily_screen_time_hours < dt$social_media_hours + dt$gaming_hours - 1e-6
  list(
    n_checked  = sum(needs_impute),
    n_violate  = sum(v[needs_impute]),
    rate       = mean(v[needs_impute]),
    # 精确对应 L3 显式处理的那种情况：屏幕缺、社交与游戏都在
    rate_screen_only = {
      m <- na_screen & !na_social & !na_gaming
...
r2   <- check_violation(d2, na_screen, na_social, na_gaming, needs_impute)
cat(sprintf("审查行数：%s\n", format(r2$n_checked, big.mark = ",")))
cat(sprintf("违反约束：%s（%.2f%%）\n", format(r2$n_violate, big.mark = ","), 100 * r2$rate))
cat(sprintf("其中「屏幕缺、社交游戏都在」这种 L3 会显式修正的情况，违反率：%.2f%%\n",
