...
Second, missingness affects every one of the twelve columns, at rates ranging from 4.18% (`age`) to 19.38% (`social_media_hours`) — not uniformly, and not summarized by the 61.06% figure above, which counts rows missing *at least one* column rather than any single column's rate. Restricted to the outcome, missingness is uninformative: across the twelve columns, the largest difference in addiction rate between the rows where a column is missing and the rows where it is observed is 0.0042 (`R/32_mcar_check.R`, reproducing the figure originally reported from `R/02_eda.R`) — small enough to be consistent with sampling noise. That check, however, establishes only a necessary condition for missingness completely at random (MCAR; Little & Rubin, *Statistical Analysis with Missing Data*): MCAR requires the missingness mechanism to be independent of the full set of feature values, observed and unobserved, not only of the outcome. These are two distinct claims: outcome-balance alone is not sufficient evidence for MCAR, only necessary.

We therefore tested the stronger claim directly. For each of the twelve columns, an xgboost classifier (Chen & Guestrin, 2016) predicts that column's missingness indicator from the other eleven raw columns, evaluated by 5-fold cross-validation on this paper's frozen folds (§3.1; `R/32_mcar_check.R`); an AUC near 0.5 is consistent with MCAR for that column, and a substantially higher AUC would indicate that missingness depends on other features' values — missing at random (MAR) rather than MCAR. Taken at face value, the result argues against MCAR: all twelve columns score well above chance, from 0.5656 to 0.7560. Decomposing the predictor set resolves why. Because the classifier's inputs are the other columns' raw values with their own missingness left as native missing data, it can exploit two different signals: whether those columns are themselves missing, and what value they hold when they are not. Restricting the same classifier to only the other columns' missingness indicators reproduces 99.1%–105.6% of the original signal on every one of the twelve columns; restricting it instead to only their observed values, on the subset of rows where all eleven are complete, collapses the AUC to 0.5012–0.5144 for every column — indistinguishable from chance in practical terms. The elevated AUC, in other words, reflects that missingness clusters across columns — some rows are systematically more complete than others, a structural fact the outcome-balance check could not have detected — and not that missingness depends on the other covariates' values. On the specific question the outcome-balance check left open, the evidence therefore supports MCAR rather than complicating it — with the qualification any test of this kind carries: it can rule out dependence on observed data, but not dependence on the value a missing cell would have taken had it been observed, which no diagnostic run on the observed data alone can rule out. Given this, and consistent with how the term is used throughout the rest of the paper, we refer to missingness under this pattern as MCAR.

Together, these facts reframe the prediction task. If the label were only weakly related to the features, a more powerful model would matter more than complete data. If missingness carried information about the label — for example, if users who did not report their screen time were disproportionately likely to be addicted — the missingness pattern itself would be a usable feature. Neither holds here: the signal is strong when the data are complete, and the incompleteness is uninformative about which rows are missing. What remains is a narrower, more concrete question: given that 61.06% of rows have some of that signal removed, how much of it can be recovered, and by what means?

This missingness-centric picture is corroborated by a separate, independent check. Comparing the training and test sets column by column, the distributions of observed values are statistically indistinguishable (Kolmogorov–Smirnov tests), while per-column missingness rates differ by up to 3.4 percentage points with no consistent direction across columns — the two sets differ in how much of each column is missing, not in what the observed values look like (`R/39_train_test_shift.R`). That the only detectable train/test difference is a missingness-rate difference, rather than a shift in the underlying feature distributions, is further evidence that missingness — not the values themselves — is the axis this dataset varies along.
