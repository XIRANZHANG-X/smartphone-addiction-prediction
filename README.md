...
地基已全部完成并实测跑通，4×4 实验网格完成 **6/14** 格。最好的单模型是 **xgboost + 完全不插补，CV AUC 0.95910**。

已经得到一个核心发现：**同一个插补方法，在 xgboost 上是最差的，在 glmnet 上是最好的**——插补的价值完全取决于下游模型能否自行表示"未知"。详见 [docs/进度.md](docs/进度.md)。
