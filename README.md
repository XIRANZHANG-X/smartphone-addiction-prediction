...
```
每日屏幕时间 >= 社交媒体时间 + 游戏时间     （451,246 行中 100% 成立）
```

**这个赌打输了**，而输的过程比赢更有价值。项目最终的核心结论是一个交互效应：

> **同一个插补方法，在梯度提升树上是最差的，在线性模型上是决定性最优的。**
> 插补的价值完全取决于下游模型能否自行表示「未知」——
> 能把未知**聚起来**统一处理的模型，聚比猜好；不能聚的模型，猜得准比猜得糙好。
...
代码架构已按[方法学审查](docs/审查响应.md)重构完毕。最大的方法论硬伤——
**xgboost 固定 600 轮、没有早停**——已修复，带来 **+0.00178** 的提升，
比四条插补线之间全部差距的一半还多。早停选出的轮数均值 1158，
说明原来的模型一直是**欠拟合**的。

14 格实验网格与后续 9 个步骤正在流水线中运行，实时状态见
[docs/进度.md](docs/进度.md)，实验数字见 [output/results.md](output/results.md)。
